class HostsController < ApplicationController
  ...
  def index
    ...
    if (request.request_method == 'GET')
			@hosts = Host.joins(:experiences).uniq.paginate(page:params[:page], per_page:3)
			...
			@hosts = Host.search(age_range[1], age_range[2], @selected_location, @selected_group, @selected_date).paginate(page:params[:page], per_page:3)
		end
  ...
  private
    ...
    def host_params
      params.require(:host).permit(:username, :email, :password, :password_confirmation, :country, :state, :image_file, :occupation, :interests, :smoker,:pets, :suburb, :latitude, :longitude, :title, :first_name, :last_name, :languages, :street_address, :intro, :neighbourhood, :additional_info, :dob, :video_url,  )
    end
